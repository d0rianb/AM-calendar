...
class SfCalendar extends StatefulWidget {
  ...
  SfCalendar({
    ...
    DateTime? minDate,
    DateTime? maxDate,
    this.appointmentTextStyle = const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w500, fontFamily: 'Roboto'),
    ...
    this.onDragEnd,
        ...
        initialDisplayDate = initialDisplayDate ?? DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 08, 45, 0),
...
class _SfCalendarState extends State<SfCalendar> with SingleTickerProviderStateMixin {
      ...
      widgets.add(MouseRegion(
          ...
          child: GestureDetector(
            child: _ScheduleAppointmentView(
                header: Container(child: CustomPaint(painter: _AgendaDateTimePainter(currentDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform), size: Size(viewPadding, appointmentViewHeaderHeight))),
                content: Container(
  ...
  Widget _getMonthOrWeekHeader(DateTime startDate, DateTime? endDate, bool isRTL, bool isMonthLabel, {double viewPadding = 0, bool isNeedTopPadding = false}) {
    ...
    if (isMonthLabel && widget.scheduleViewMonthHeaderBuilder != null) {
      final ScheduleViewMonthHeaderDetails details = ScheduleViewMonthHeaderDetails(DateTime(startDate.year, startDate.month, 1), Rect.fromLTWH(0, 0, _minWidth, widget.scheduleViewSettings.monthHeaderSettings.height));
  ...
  Widget addAgendaWithLoadMore(double height, bool isRTL) {
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      if (_nextDates.isNotEmpty && isNewDatesAdded) {
        ...
        _headerUpdateNotifier.value = _useMobilePlatformUI ? date : DateTimeHelper.getDateTimeValue(getValidDate(_minDate, _maxDate, date));
      }
    }
    ...
    if (_nextDates.isNotEmpty && _agendaScrollController!.initialScrollOffset == 0 && !isInitialLoadMore && !_isNeedLoadMore && !_isScheduleStartLoadMore && isSameDate(visibleMaxDate, _scheduleMaxDate) && isSameDate(visibleMinDate, _scheduleMinDate)) {
      ...
      while (viewStartDate.isBefore(scheduleDisplayDate) && !isSameDate(viewStartDate, scheduleDisplayDate)) {
        ...
        if (appointmentCollection.isNotEmpty) {
          ...
          /// position.
          initialScrolledPosition += appointmentHeight + (_useMobilePlatformUI ? padding : 0);
        ...
        child: _PopupWidget(
            ...
            child: Container(
                ...
                child: Material(
                  ...
                  child: ListView(padding: const EdgeInsets.all(0), controller: ScrollController(initialScrollOffset: 0), children: children),
                ))));
  }
        ...
        child: Stack(children: <Widget>[
          ...
          Positioned(
              ...
              child: MouseRegion(
                  ...
                  child: GestureDetector(
                    child: ListView(padding: const EdgeInsets.all(0.0), physics: const ClampingScrollPhysics(), controller: _resourcePanelScrollController, scrollDirection: Axis.vertical, children: <Widget>[
  ...
  Widget _addAgendaView(double height, double startPosition, double width, bool isRTL) {
    ...
    if (agendaAppointments.isNotEmpty) {
      ...
      painterHeight = (((count * (allDayAppointmentHeight + topPadding)) + ((agendaAppointments.length - count) * (appointmentHeight + topPadding))).toDouble()) + bottomPadding;
    }
...
class _PopupWidgetState extends State<_PopupWidget> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return ScaleTransition(alignment: widget.alignment, scale: _animation, child: FadeTransition(opacity: _animation, child: widget.child));
  }
}

@immutable
class _CalendarHeaderView extends StatefulWidget {
  const _CalendarHeaderView(this.visibleDates, this.headerStyle, this.currentDate, this.view, this.numberOfWeeksInView, this.calendarTheme, this.isRTL, this.locale, this.showNavigationArrow, this.controller, this.maxDate, this.minDate, this.width, this.height, this.nonWorkingDays, this.navigationDirection, this.showDatePickerButton, this.isPickerShown, this.allowedViews, this.allowViewNavigation,
      this.localizations, this.removePicker, this.valueChangeNotifier, this.viewChangeNotifier, this.headerTapCallback, this.headerLongPressCallback, this.todayHighlightColor, this.textScaleFactor, this.isMobilePlatform, this.headerDateFormat, this.enableInteraction, this.todayTextStyle, this.showWeekNumber, this.weekNumberStyle, this.timelineMonthWeekNumberNotifier);
  ...
  Widget build(BuildContext context) {
    ...
    if (isNeedViewSwitchOption) {
      ...
      if (useMobilePlatformUI) {
        ...
        double totalWidth = widget.width - totalArrowWidth - dividerWidth - todayIconWidth;
        ...
        for (int i = 0; i < allowedViewsLength; i++) {
          ...
          maxCalendarViewHeight = maxCalendarViewHeight > calendarViewSize.height ? maxCalendarViewHeight : calendarViewSize.height;
        ...
        if (allowedViewsWidth < totalWidth) {
          ...
          maxHeaderHeight = maxCalendarViewHeight > maxHeaderHeight ? maxCalendarViewHeight : maxHeaderHeight;
          maxHeaderHeight = maxHeaderHeight > widget.height ? widget.height : maxHeaderHeight;
          calendarViewWidth = calendarViewSize.width + padding + headerIconTextWidth;
                  ...
                  child: Container(
                      ...
                      child: Row(
                          ...
                          children: widget.showDatePickerButton
                              ? <Widget>[
                                  Flexible(child: Text(headerString, style: headerTextStyle, maxLines: 1, semanticsLabel: headerString + ' ' + (widget.isPickerShown ? 'hide date picker' : 'show date picker'), overflow: TextOverflow.clip, softWrap: false, textDirection: TextDirection.ltr)),
                                  Icon(
            ...
            child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                      ...
                      width: isCenterAlignment && headerTextWidth > 200 ? 200 : headerTextWidth,
                      height: headerHeight,
                      alignment: Alignment.center,
                      padding: const EdgeInsets.symmetric(horizontal: 5),
                      child: Row(
    ...
    final Widget weekNumberWidget = weekNumberEnabled
        ? Container(
            ...
            alignment: _getHeaderAlignment(),
            child: Container(
                decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(padding)), shape: BoxShape.rectangle, color: weekNumberBackgroundColor),
                alignment: Alignment.center,
    ...
    if (widget.headerStyle.textAlign == TextAlign.left || widget.headerStyle.textAlign == TextAlign.start) {
      ...
      return Row(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.center, children: rowChildren);
    } else if (widget.headerStyle.textAlign == TextAlign.right || widget.headerStyle.textAlign == TextAlign.end) {
      if (widget.isMobilePlatform) {
        rowChildren = <Widget>[
          leftArrow,
      ...
      case CalendarView.schedule:
        {
          ...
          return DateFormat(monthFormat, widget.locale).format(widget.valueChangeNotifier.value!).toString() + ' ' + widget.valueChangeNotifier.value!.year.toString();
        }
      case CalendarView.month:
      case CalendarView.timelineMonth:
        {
          ...
          if (widget.numberOfWeeksInView != 6 && startDate.month != endDate.month) {
            if (headerDateFormat != null) {
              return DateFormat(headerDateFormat, widget.locale).format(startDate).toString() + ' - ' + DateFormat(headerDateFormat, widget.locale).format(endDate).toString();
            }
            monthFormat = 'MMM';
            return DateFormat(monthFormat, widget.locale).format(startDate).toString() + ' ' + startDate.year.toString() + ' - ' + DateFormat(monthFormat, widget.locale).format(endDate).toString() + ' ' + endDate.year.toString();
          }

          if (headerDateFormat != null) {
            return DateFormat(headerDateFormat, widget.locale).format(widget.currentDate!).toString();
          }
          return DateFormat(monthFormat, widget.locale).format(widget.currentDate!).toString() + ' ' + widget.currentDate!.year.toString();
        }
      case CalendarView.day:
      case CalendarView.week:
      case CalendarView.workWeek:
      case CalendarView.timelineDay:
        {
          ...
          if (headerDateFormat != null) {
            return capitalize(DateFormat(headerDateFormat, widget.locale).format(headerDate).toString());
          }
      ...
      case CalendarView.timelineWorkWeek:
        {
          ...
          final String endText = endDate.day.toString() + ' ' + DateFormat(monthFormat, widget.locale).format(endDate).toString() + ' ' + endDate.year.toString();
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    if (!isMonthLabel) {
      if (!isDisplayDate) {
        ...
        accessibilityText = DateFormat('MMMM dd', locale).format(startDate).toString() + 'to' + DateFormat('MMMM dd', locale).format(endDate!).toString();
      } else {
...
class _AgendaDateTimePainter extends CustomPainter {
  ...
  void _addDayLabelForMobile(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday, bool isMobile) {
    ...
    TextSpan span = TextSpan(text: DateFormat(dayTextFormat, locale).format(selectedDate!).toUpperCase().toString(), style: dayTextStyle);
    _updateTextPainter(span);
  ...
  void _addDayLabelForWeb(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday) {
    ...
    final String dayText = DateFormat(isRTL ? scheduleViewSettings!.dayHeaderSettings.dayFormat + ', MMM' : 'MMM, ' + scheduleViewSettings!.dayHeaderSettings.dayFormat, locale).format(selectedDate!).toUpperCase().toString();
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    if (selectedDate == null) {
      ...
      semanticsBuilder.add(CustomPainterSemantics(
        ...
        properties: SemanticsProperties(
          label: DateFormat('EEEEE').format(selectedDate!).toString() + DateFormat('dd MMMM yyyy').format(selectedDate!).toString(),
          textDirection: TextDirection.ltr,
        ),
      ));
    }
...
Map<CalendarView, String> _getCalendarViewsText(SfLocalizations localizations, [bool isUpperCase = false]) {
  ...
  calendarViews[CalendarView.day] = localizations.allowedViewDayLabel.isNotEmpty
      ...
      : isUpperCase
          ? 'DAY'
          : 'Day';
  calendarViews[CalendarView.week] = localizations.allowedViewWeekLabel.isNotEmpty
      ? localizations.allowedViewWeekLabel
      : isUpperCase
          ? 'WEEK'
          : 'Week';
  calendarViews[CalendarView.workWeek] = localizations.allowedViewWorkWeekLabel.isNotEmpty
      ? localizations.allowedViewWorkWeekLabel
      : isUpperCase
          ? 'WORK WEEK'
          : 'Work Week';
  calendarViews[CalendarView.timelineDay] = localizations.allowedViewTimelineDayLabel.isNotEmpty
      ? localizations.allowedViewTimelineDayLabel
      : isUpperCase
          ? 'TIMELINE DAY'
          : 'Timeline Day';
  calendarViews[CalendarView.timelineWeek] = localizations.allowedViewTimelineWeekLabel.isNotEmpty
      ? localizations.allowedViewTimelineWeekLabel
      : isUpperCase
          ? 'TIMELINE WEEK'
          : 'Timeline Week';
  calendarViews[CalendarView.timelineMonth] = localizations.allowedViewTimelineMonthLabel.isNotEmpty
      ? localizations.allowedViewTimelineMonthLabel
      : isUpperCase
          ? 'TIMELINE MONTH'
          : 'Timeline Month';
  calendarViews[CalendarView.timelineWorkWeek] = localizations.allowedViewTimelineWorkWeekLabel.isNotEmpty
      ? localizations.allowedViewTimelineWorkWeekLabel
      : isUpperCase
          ? 'TIMELINE WORK WEEK'
          : 'Timeline Work Week';
  calendarViews[CalendarView.month] = localizations.allowedViewMonthLabel.isNotEmpty
      ? localizations.allowedViewMonthLabel
      : isUpperCase
          ? 'MONTH'
          : 'Month';
  calendarViews[CalendarView.schedule] = localizations.allowedViewScheduleLabel.isNotEmpty
