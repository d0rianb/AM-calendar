...
class _CustomCalendarScrollViewState extends State<CustomCalendarScrollView> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isDayView = CalendarViewHelper.isDayView(widget.view, widget.calendar.timeSlotViewSettings.numberOfDaysInView, widget.calendar.timeSlotViewSettings.nonWorkingDays, widget.calendar.monthViewSettings.numberOfWeeksInView);
    final bool isTimelineView = CalendarViewHelper.isTimelineView(widget.view);
          ...
          Positioned(
              ...
              child: IgnorePointer(
                  child: RepaintBoundary(
                      child: _DraggingAppointmentWidget(_dragDetails, widget.isRTL, widget.textScaleFactor, widget.isMobilePlatform, AppointmentHelper.getAppointmentTextStyle(widget.calendar.appointmentTextStyle, widget.view), widget.calendar.dragAndDropSettings, widget.view, _updateCalendarStateDetails.allDayPanelHeight, viewHeaderHeight, timeLabelWidth, resourceItemHeight, widget.calendarTheme,
                          widget.calendar, widget.width, widget.height))))
        ],
      ),
    );
  }
  ...
  void _handlePointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent && viewKey != null) {
      ...
      if (event.scrollDelta.dy.abs() > event.scrollDelta.dx.abs() && viewKey._timelineViewVerticalScrollController!.position.maxScrollExtent == 0) {
  ...
  void _updateVisibleDates() {
    ...
    final int visibleDatesCount = DateTimeHelper.getViewDatesCount(widget.view, widget.calendar.monthViewSettings.numberOfWeeksInView, widget.calendar.timeSlotViewSettings.numberOfDaysInView, nonWorkingDays);
    ...
    final DateTime prevDate = DateTimeHelper.getPreviousViewStartDate(widget.view, widget.calendar.monthViewSettings.numberOfWeeksInView, currentDate, visibleDatesCount, nonWorkingDays);
    final DateTime nextDate = DateTimeHelper.getNextViewStartDate(widget.view, widget.calendar.monthViewSettings.numberOfWeeksInView, currentDate, visibleDatesCount, nonWorkingDays);
  ...
  void _updateNextViewVisibleDates() {
    ...
    if (widget.isRTL) {
      currentViewDate = DateTimeHelper.getPreviousViewStartDate(widget.view, widget.calendar.monthViewSettings.numberOfWeeksInView, currentViewDate, visibleDatesCount, nonWorkingDays);
    } else {
  ...
  KeyEventResult _updatePageUpAndDown(RawKeyEvent event, _CalendarViewState currentViewState, bool isResourceEnabled) {
    ...
    final bool isDayView = CalendarViewHelper.isDayView(widget.controller.view!, widget.calendar.timeSlotViewSettings.numberOfDaysInView, widget.calendar.timeSlotViewSettings.nonWorkingDays, widget.calendar.monthViewSettings.numberOfWeeksInView);
...
class _CalendarViewState extends State<_CalendarView> with TickerProviderStateMixin {
      ...
      child: Stack(
        children: <Widget>[
          GestureDetector(
            ...
            child: SizedBox(height: widget.height, width: widget.width, child: _addDayView(widget.width, _timeIntervalHeight * _horizontalLinesCount!, _isRTL, widget.locale, isCurrentView)),
          ),
          _getResizeShadowView()
        ],
      ),
  ...
  void _updateTimeSlotView(_CalendarView oldWidget) {
    ...
    if (!CalendarViewHelper.isDayView(widget.view, widget.calendar.timeSlotViewSettings.numberOfDaysInView, widget.calendar.timeSlotViewSettings.nonWorkingDays, widget.calendar.monthViewSettings.numberOfWeeksInView) && _allDayHeight == 0) {
  ...
  Widget _addAllDayAppointmentPanel(SfCalendarThemeData calendarTheme, bool isCurrentView) {
    ...
    if (_allDayHeight == 0 || (!isDayView && widget.visibleDates != _updateCalendarStateDetails.currentViewVisibleDates)) {
  ...
  Widget _getAllDayLayout(double timeLabelWidth, double panelHeight, double allDayExpanderHeight, bool isCurrentView) {
    ...
    final Widget allDayLayout = AllDayAppointmentLayout(
        ...
        widget.isMobilePlatform,
        widget.width,
        (isDayView && _updateCalendarStateDetails.allDayPanelHeight < _allDayHeight) || !isCurrentView ? _allDayHeight : _updateCalendarStateDetails.allDayPanelHeight,
        widget.localizations,
          ...
          child: IgnorePointer(
              ...
              child: RepaintBoundary(
                ...
                painter: _ResizingAppointmentPainter(
                    _resizingDetails, _isRTL, widget.textScaleFactor, widget.isMobilePlatform, AppointmentHelper.getAppointmentTextStyle(widget.calendar.appointmentTextStyle, widget.view), allDayPanelHeight, viewHeaderHeight, timeLabelWidth, _timeIntervalHeight, _scrollController, widget.calendar.dragAndDropSettings, widget.view, _mouseCursor, weekNumberPanelWidth, widget.calendarTheme),
              ))),
        ));
  }
      ...
      children: <Widget>[
        ...
        Positioned(
            ...
            child: Scrollbar(
              ...
              child: ListView(padding: EdgeInsets.zero, controller: _scrollController, physics: const ClampingScrollPhysics(), children: <Widget>[
    ...
    return Stack(children: <Widget>[
      ...
      Positioned(
          ...
          child: Scrollbar(
            controller: _scrollController,
            thumbVisibility: !widget.isMobilePlatform,
            child: ListView(padding: EdgeInsets.zero, controller: _scrollController, scrollDirection: Axis.horizontal, physics: const _CustomNeverScrollableScrollPhysics(), children: <Widget>[
              SizedBox(
                  width: width,
                  child: Stack(children: <Widget>[
                    Scrollbar(
                        ...
                        thumbVisibility: !widget.isMobilePlatform,
                        child: ListView(padding: EdgeInsets.zero, controller: _timelineViewVerticalScrollController, physics: isResourceEnabled ? const ClampingScrollPhysics() : const NeverScrollableScrollPhysics(), children: <Widget>[
                          Stack(children: <Widget>[
                            RepaintBoundary(
                                child: _CalendarMultiChildContainer(
                              width: width,
  ...
  CalendarDetails? _getDetailsForMonth(Offset position) {
    ...
    if (appointmentView == null) {
      ...
      if (!CalendarViewHelper.isCurrentMonthDate(widget.calendar.monthViewSettings.numberOfWeeksInView, widget.calendar.monthViewSettings.showTrailingAndLeadingDates, currentMonth, getDate)) {
    ...
    final List<dynamic> selectedAppointments = appointmentView == null || isMoreTapped ? _getSelectedAppointments(getDate) : <dynamic>[CalendarViewHelper.getAppointmentDetail(appointmentView.appointment!, widget.calendar.dataSource)];
  ...
  CalendarDetails? _getDetailsForTimeline(Offset position) {
    ...
    if (appointmentView == null) {
      ...
      return CalendarDetails(<dynamic>[CalendarViewHelper.getAppointmentDetail(appointmentView.appointment!, widget.calendar.dataSource)], getDate, CalendarElement.appointment, calendarResource);
    }
  }
  ...
  CalendarDetails? _getDetailsForDay(Offset position) {
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (isDayView && ((!_isRTL && xDetails <= timeLabelWidth) || (_isRTL && widget.width - xDetails <= timeLabelWidth)) && yDetails < CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view)) {
        /// Return calendar details while the [getCalendarDetailsAtOffset]
      ...
      if (appointmentView != null && (yPosition < allDayHeight - kAllDayAppointmentHeight || _updateCalendarStateDetails.allDayPanelHeight <= allDayHeight || appointmentView.position + 1 >= appointmentView.maxPositions)) {
        ...
        /// view.
        return CalendarDetails(widget.calendar.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.calendar.dataSource!) ? CalendarViewHelper.getCustomAppointments(moreRegionAppointments, widget.calendar.dataSource) : moreRegionAppointments, selectedDate, CalendarElement.moreAppointmentRegion, null);
  ...
  double _getTimeIntervalHeight(SfCalendar calendar, CalendarView view, double width, double height, int visibleDatesCount, double allDayHeight, bool isMobilePlatform) {
    ...
    final bool isDayView = CalendarViewHelper.isDayView(view, calendar.timeSlotViewSettings.numberOfDaysInView, calendar.timeSlotViewSettings.nonWorkingDays, calendar.monthViewSettings.numberOfWeeksInView);
  ...
  Widget _getTimelineViewHeader(double width, double height, String locale) {
    ...
    return ListView(padding: EdgeInsets.zero, controller: _timelineViewHeaderScrollController, scrollDirection: Axis.horizontal, physics: const NeverScrollableScrollPhysics(), children: <Widget>[
      CustomPaint(
...
class _ViewHeaderViewPainter extends CustomPainter {
  ...
  void _addViewHeaderForTimeSlotViews(Canvas canvas, Size size, TextStyle viewHeaderDayStyle, TextStyle viewHeaderDateStyle, double width, DateTime today) {
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      if (!isDayView && showWeekNumber && ((currentDate.weekday == DateTime.monday) || (view == CalendarView.workWeek && timeSlotViewSettings.nonWorkingDays.contains(DateTime.monday) && i == visibleDatesLength ~/ 2))) {
  ...
  String _updateViewHeaderFormat(String dayFormat, String dayText) {
    switch (view) {
      ...
      case CalendarView.workWeek:
        {
          if (!CalendarViewHelper.isDayView(view, timeSlotViewSettings.numberOfDaysInView, timeSlotViewSettings.nonWorkingDays, monthViewSettings.numberOfWeeksInView) && (dayFormat == 'EE' && (locale.contains('en')))) {
            return dayText[0];
          }
          break;
        }
  ...
  List<CustomPainterSemantics> _getSemanticsForDayHeader(Size size) {
    ...
    for (int i = 0; i < visibleDates.length; i++) {
      ...
      if (showWeekNumber && ((visibleDate.weekday == DateTime.monday && !isDayView) || (view == CalendarView.workWeek && timeSlotViewSettings.nonWorkingDays.contains(DateTime.monday) && i == visibleDates.length ~/ 2))) {
