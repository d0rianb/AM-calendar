...
class SfCalendar extends StatefulWidget {
  ...
  SfCalendar({
    ...
    DateTime? minDate,
    DateTime? maxDate,
    this.appointmentTextStyle = const TextStyle(color: Colors.white, fontSize: -1, fontWeight: FontWeight.w500, fontFamily: 'Roboto'),
    ...
    this.onDragEnd,
        ...
        initialDisplayDate = initialDisplayDate ?? DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 08, 45),
  ...
  ///
  /// ```
  static List<DateTime> getRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate, {DateTime? specificStartDate, DateTime? specificEndDate}) {
    assert(specificStartDate == null || specificEndDate == null || CalendarViewHelper.isSameOrBeforeDateTime(specificEndDate, specificStartDate));
    assert(specificStartDate == null || specificEndDate == null || CalendarViewHelper.isSameOrAfterDateTime(specificStartDate, specificEndDate));
    return RecurrenceHelper.getRecurrenceDateTimeCollection(rRule, recurrenceStartDate, specificStartDate: specificStartDate, specificEndDate: specificEndDate);
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<ScheduleViewMonthHeaderBuilder>('scheduleViewMonthHeaderBuilder', scheduleViewMonthHeaderBuilder));
...
class _SfCalendarState extends State<SfCalendar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _currentDate = DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, _controller.displayDate ?? widget.initialDisplayDate));
  ...
  void didChangeDependencies() {
    ...
    _calendarTheme = _getThemeDataValue(calendarThemeData, Theme.of(context).colorScheme);
  ...
  void didUpdateWidget(SfCalendar oldWidget) {
    ...
    if (oldWidget.controller == widget.controller && widget.controller != null && oldWidget.controller!.displayDate != widget.controller!.displayDate) {
    ...
    if ((widget.loadMoreWidgetBuilder == null && oldWidget.loadMoreWidgetBuilder != null) || (widget.loadMoreWidgetBuilder != null && oldWidget.loadMoreWidgetBuilder == null)) {
    ...
    if (_view == CalendarView.month && widget.monthViewSettings.showTrailingAndLeadingDates != oldWidget.monthViewSettings.showTrailingAndLeadingDates) {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      _fadeInController ??= AnimationController(duration: Duration(milliseconds: _isMobilePlatform ? 500 : 600), vsync: this)..addListener(_updateFadeAnimation);
      ...
      if (_view == CalendarView.schedule && _actualWidth != null && _useMobilePlatformUI != CalendarViewHelper.isMobileLayoutUI(_actualWidth!, _isMobilePlatform) && _nextDates.isNotEmpty) {
        _agendaScrollController?.removeListener(_handleScheduleViewScrolled);
        _initScheduleViewProperties();
      }
      ...
      final double agendaHeight = _view == CalendarView.month && widget.monthViewSettings.showAgenda ? _getMonthAgendaHeight() : 0;
      ...
      case CalendarView.month:
        {
          if (widget.monthViewSettings.showAgenda) {
            ...
            if (position.dy > _minHeight - agendaHeight) {
              ...
              if (_selectedDate != null) {
                currentSelectedDate = isDateWithInDateRange(widget.minDate, widget.maxDate, _selectedDate) && !CalendarViewHelper.isDateInDateCollection(_blackoutDates, _selectedDate!) ? _selectedDate : null;
              }
              ...
              final List<dynamic> selectedAppointments = _getSelectedAppointments(Offset(position.dx, position.dy - (_minHeight - agendaHeight)), currentSelectedDate);

              /// Return calendar details while the [getCalendarDetailsAtOffset]
              /// position placed on agenda in month view.
              return CalendarDetails(selectedAppointments, currentSelectedDate, selectedAppointments.isNotEmpty ? CalendarElement.appointment : CalendarElement.agenda, null);
            }
          }
      ...
      case CalendarView.timelineMonth:
        {
          ...
          if ((!_isRTL && updatedPosition.dx < resourceViewSize) || (_isRTL && updatedPosition.dx > _minWidth - resourceViewSize)) {
            ...
            final double resourceItemHeight = CalendarViewHelper.getResourceItemHeight(resourceViewSize, _minHeight - top, widget.resourceViewSettings, _resourceCollection!.length);
          ...
          return (_customScrollViewKey.currentWidget! as CustomCalendarScrollView).getCalendarDetails(Offset(_isRTL ? updatedPosition.dx : updatedPosition.dx - resourceViewSize, updatedPosition.dy));
        }
  ...
  CalendarDetails? _getScheduleViewDetails(Offset position) {
    ...
    /// date.
    final List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(isSameOrAfterDate(_minDate, startDate) ? startDate : _minDate!, isSameOrBeforeDate(_maxDate, endDate) ? endDate : _maxDate!, _appointments, widget.timeZone, false, canCreateNewAppointment: false);
    appointmentCollection.sort((CalendarAppointment app1, CalendarAppointment app2) => app1.actualStartTime.compareTo(app2.actualStartTime));

    /// Get the collection of appointment collection listed by date.
    final Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(appointmentCollection, startDate, endDate);
    ...
    bool isNeedCurrentDateHighlight = isDateWithInDateRange(startDate, endDate, scheduleCurrentDate) && !isSameDate(scheduleDisplayDate, scheduleCurrentDate);
    ...
    /// view.
    final bool isNeedMonthBuilder = _useMobilePlatformUI && (prevEndDate.month != startDate.month || prevEndDate.year != startDate.year);

    final double appointmentViewHeight = CalendarViewHelper.getScheduleAppointmentHeight(null, widget.scheduleViewSettings);
    final double allDayAppointmentHeight = CalendarViewHelper.getScheduleAllDayAppointmentHeight(null, widget.scheduleViewSettings);
    ...
    if (_useMobilePlatformUI) {
      ...
      if (topPosition <= yPosition && topPosition + weekHeaderHeight > yPosition) {
        ...
        return CalendarDetails(null, DateTime(startDate.year, startDate.month, startDate.day), CalendarElement.viewHeader, null);
      }
    ...
    /// views.
    bool isNeedInBetweenMonthBuilder = _useMobilePlatformUI && (startDate.month != (isSameOrBeforeDate(_maxDate, endDate) ? endDate : _maxDate!).month);

    /// Check the end date month have appointments or not.
    bool isNextMonthHasNoAppointment = false;
    if (isNeedInBetweenMonthBuilder) {
      final DateTime? lastAppointmentDate = dateAppointmentKeys.isNotEmpty ? dateAppointmentKeys[dateAppointmentKeys.length - 1] : null;
      ...
      isNextMonthHasNoAppointment = lastAppointmentDate == null || (lastAppointmentDate.month != endDate.month && nextWeekDate != null && nextWeekDate.month == endDate.month && nextWeekDate.year == endDate.year);
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      /// before month header.
      if (!isDisplayDateHighlightAdded && currentDate.isAfter(scheduleDisplayDate) && currentDate.month != scheduleDisplayDate.month) {
      ...
      /// before month header.
      if (!isCurrentDateHighlightAdded && currentDate.isAfter(scheduleCurrentDate) && currentDate.month != scheduleCurrentDate.month) {
      ...
      /// point.
      if (!isMonthHeaderAddedOnView && _useMobilePlatformUI && (startDate.month != currentDate.month || startDate.year != currentDate.year)) {
      ...
      final double panelHeight = totalPadding + ((eventsCount - allDayEventCount) * appointmentViewHeight) + (allDayEventCount * allDayAppointmentHeight);
      ...
      if (topPosition <= yPosition && yPosition < topPosition + panelHeight + appointmentViewPadding + dividerHeight) {
        currentAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => app1.actualStartTime.compareTo(app2.actualStartTime));
        currentAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
        currentAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isSpanned, app2.isSpanned));
        ...
        if ((!_isRTL && viewPadding >= position.dx) || (_isRTL && _minWidth - viewPadding < position.dx)) {
          ...
          /// Return calendar details while the [getCalendarDetailsAtOffset]
          /// position placed on view header in schedule view.
          return CalendarDetails(widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(currentAppointments, widget.dataSource) : currentAppointments, DateTime(currentDate.year, currentDate.month, currentDate.day), CalendarElement.viewHeader, null);
        } else {
          ...
          for (int k = 0; k < currentAppointments.length; k++) {
            ...
            final double currentAppointmentHeight = (_useMobilePlatformUI && _isAllDayAppointmentView(appointment) ? allDayAppointmentHeight : appointmentViewHeight) + padding;
            if (currentYPosition <= yPosition && currentYPosition + currentAppointmentHeight > yPosition) {
              ...
              return CalendarDetails(widget.dataSource != null && !AppointmentHelper.isCalendarAppointment(widget.dataSource!) ? CalendarViewHelper.getCustomAppointments(selectedAppointment, widget.dataSource) : selectedAppointment, DateTime(currentDate.year, currentDate.month, currentDate.day), CalendarElement.appointment, null);
            }
    ...
    if (_useMobilePlatformUI && isNeedInBetweenMonthBuilder && isNextMonthHasNoAppointment && isSameOrBeforeDate(_maxDate, endDate)) {
  ...
  SfCalendarThemeData _getThemeDataValue(SfCalendarThemeData calendarThemeData, ColorScheme colorScheme) {
    return calendarThemeData.copyWith(
        ...
        weekNumberBackgroundColor: calendarThemeData.weekNumberBackgroundColor ?? colorScheme.onSurface.withOpacity(0.04),
        ...
        trailingDatesTextStyle: calendarThemeData.trailingDatesTextStyle ?? TextStyle(color: colorScheme.onSurface.withOpacity(0.54), fontSize: 13, fontFamily: 'Roboto'),
        leadingDatesTextStyle: calendarThemeData.leadingDatesTextStyle ?? TextStyle(color: colorScheme.onSurface.withOpacity(0.54), fontSize: 13, fontFamily: 'Roboto'),
  ...
  void _updateVisibleAppointments() async {
    ...
    if (_view != CalendarView.schedule) {
      ...
      if (_view == CalendarView.month && !CalendarViewHelper.isLeadingAndTrailingDatesVisible(widget.monthViewSettings.numberOfWeeksInView, widget.monthViewSettings.showTrailingAndLeadingDates)) {
      ...
      final List<CalendarAppointment> tempVisibleAppointment =
          ...
          await AppointmentHelper.getVisibleAppointments(viewStartDate, viewEndDate, _appointments, widget.timeZone, _view == CalendarView.month || CalendarViewHelper.isTimelineView(_view));
  ...
  void _handleScheduleViewScrolled() {
    ...
    if (scrolledPosition >= 0) {
      for (int i = 0; i < _forwardWidgetHeights.length; i++) {
        ...
        if (scrolledPosition >= widgetPosition && scrolledPosition < widgetHeight) {
          ...
          if (currentViewDate.month != _headerUpdateNotifier.value!.month || currentViewDate.year != _headerUpdateNotifier.value!.year) {
    ...
    if (_agendaScrollController!.hasClients && _agendaScrollController!.position.atEdge && (_agendaScrollController!.position.minScrollExtent != 0 || _agendaScrollController!.position.maxScrollExtent != 0) && widget.loadMoreWidgetBuilder != null && !_isNeedLoadMore && !_isScheduleStartLoadMore) {
  ...
  void _updateSelectionChangedCallback() {
    ...
    final bool isResourceEnabled = CalendarViewHelper.isResourceEnabled(widget.dataSource, _view);
    CalendarViewHelper.raiseCalendarSelectionChangedCallback(widget, _controller.selectedDate, isResourceEnabled ? widget.dataSource!.resources![0] : null);
  }
  ...
  void _updateDisplayDate() {
    ...
    switch (_view) {
      ...
      case CalendarView.timelineMonth:
        {
          if (isSameDate(_currentDate, _controller.displayDate) || isDateWithInDateRange(_currentViewVisibleDates[0], _currentViewVisibleDates[_currentViewVisibleDates.length - 1], _controller.displayDate)) {
  ...
  void _updateCurrentVisibleDates() {
    final List<int>? nonWorkingDays = (_view == CalendarView.workWeek || _view == CalendarView.timelineWorkWeek) ? widget.timeSlotViewSettings.nonWorkingDays : null;
    final int visibleDatesCount = DateTimeHelper.getViewDatesCount(_view, widget.monthViewSettings.numberOfWeeksInView, widget.timeSlotViewSettings.numberOfDaysInView, nonWorkingDays);
  ...
  void _dataSourceChangedListener(CalendarDataSourceAction type, List<dynamic> data) {
    ...
    switch (type) {
      ...
      case CalendarDataSourceAction.add:
        {
          final List<CalendarAppointment> collection = AppointmentHelper.generateCalendarAppointments(widget.dataSource, widget.timeZone, data);
          ...
          if (_view != CalendarView.schedule) {
            ...
            visibleAppointmentCollection.addAll(AppointmentHelper.getVisibleAppointments(viewStartDate, viewEndDate, collection, widget.timeZone, _view == CalendarView.month || CalendarViewHelper.isTimelineView(_view)));
          }
  ...
  void _loadScheduleViewAppointmentDates(List<CalendarAppointment> collection) {
    ...
    if (_view != CalendarView.schedule || _nextDates.isEmpty || (!widget.scheduleViewSettings.hideEmptyScheduleWeek && _useMobilePlatformUI)) {
      return;
    }
    ...
    final DateTime endDate = addDuration(_nextDates[_nextDates.length - 1], const Duration(days: DateTime.daysPerWeek - 1)) as DateTime;
    for (int i = 0; i < collection.length; i++) {
      ...
      final Duration difference = AppointmentHelper.getDifference(appointment.actualStartTime, appointment.actualEndTime);
      if (appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) {
        /// Calculate the occurrence dates collection.
        recursiveDates = RecurrenceHelper.getRecurrenceDateTimeCollection(appointment.recurrenceRule!, appointment.actualStartTime, recurrenceDuration: difference, specificStartDate: startDate, specificEndDate: endDate);
        if (appointment.recurrenceExceptionDates != null) {
          for (int i = 0; i < appointment.recurrenceExceptionDates!.length; i++) {
            recursiveExceptionDates.add(AppointmentHelper.convertTimeToAppointmentTimeZone(appointment.recurrenceExceptionDates![i], '', widget.timeZone));
          }
        }
      } else {
        if (!isDateWithInDateRange(startDate, endDate, appointment.actualStartTime) && !isDateWithInDateRange(startDate, endDate, appointment.actualEndTime)) {
          continue;
        }
  ...
  void _removeScheduleViewAppointmentDates() {
    if (_view != CalendarView.schedule || (!widget.scheduleViewSettings.hideEmptyScheduleWeek && _useMobilePlatformUI)) {
      return;
    }
    ...
    for (int i = 0; i < _previousDates.length; i++) {
      ...
      if (!_isAppointmentBetweenDates(_appointments, weekStartDate, weekEndDate, widget.timeZone) && !isDateWithInDateRange(weekStartDate, weekEndDate, DateTime.now()) && !isDateWithInDateRange(weekStartDate, weekEndDate, _scheduleDisplayDate)) {
  ...
  DateTime _updateCurrentDate(CalendarView view) {
    ...
    if (_selectedDate != null && isDateWithInDateRange(visibleStartDate, visibleEndDate, _selectedDate)) {
      if (isMonthView) {
        return DateTime(_selectedDate!.year, _selectedDate!.month, _selectedDate!.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
      } else {
        return _selectedDate!;
      }
    } else if (isDateWithInDateRange(visibleStartDate, visibleEndDate, DateTime.now())) {
      final DateTime date = DateTime.now();
      return DateTime(date.year, date.month, date.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
    } else {
      if (isMonthView) {
        if (widget.monthViewSettings.numberOfWeeksInView > 0 && widget.monthViewSettings.numberOfWeeksInView < 6) {
          return visibleStartDate;
        }
        return DateTime(_currentDate.year, _currentDate.month, 1, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
      } else {
  ...
  void _updateAppointmentPositionAndMaxPosition(List<List<AppointmentView>> allDayAppointmentView) {
    for (int i = 0; i < allDayAppointmentView.length; i++) {
      ...
      if (intersectingAppointments.isNotEmpty) {
        final int maxPosition = intersectingAppointments.reduce((AppointmentView currentAppView, AppointmentView nextAppView) => currentAppView.position > nextAppView.position ? currentAppView : nextAppView).position + 1;
  ...
  void _updateAllDayAppointment() {
    ...
    for (final CalendarAppointment appointment in _visibleAppointments) {
      if (appointment.isAllDay || AppointmentHelper.getDifference(appointment.actualStartTime, appointment.actualEndTime).inDays > 0) {
    ...
    _allDayAppointmentViewCollection.sort((AppointmentView app1, AppointmentView app2) {
      if (app1.appointment != null && app2.appointment != null) {
        return AppointmentHelper.getDifference(app2.appointment!.startTime, app2.appointment!.endTime) > AppointmentHelper.getDifference(app1.appointment!.startTime, app1.appointment!.endTime) ? 1 : 0;
      }
  ...
  void _updateAllDayPanelHeight() {
    ...
    if (_allDayAppointmentViewCollection.isNotEmpty) {
      maxPosition = _allDayAppointmentViewCollection.reduce((AppointmentView currentAppView, AppointmentView nextAppView) => currentAppView.maxPositions > nextAppView.maxPositions ? currentAppView : nextAppView).maxPositions;
    }
  ...
  void _updateMouseHoverPosition(Offset globalPosition, bool isScheduleDisplayDate, bool isRTL, DateTime? currentDate, double? startPosition, [double padding = 0, bool isResourceEnabled = false]) {
    ...
    if (localPosition.dy < widget.headerHeight) {
      ...
      if (isResourceEnabled && ((isRTL && localPosition.dx > (_minWidth - widget.resourceViewSettings.size)) || (!isRTL && localPosition.dx < widget.resourceViewSettings.size)) && localPosition.dy > startPosition! && (CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.onTap) || CalendarViewHelper.shouldRaiseCalendarLongPressCallback(widget.onLongPress))) {
  ...
  void _pointerEnterEvent(PointerEnterEvent event, bool isScheduleDisplayDate, bool isRTL, [DateTime? currentDate, double? startPosition, double padding = 0, bool resourceEnabled = false]) {
    _updateMouseHoverPosition(event.position, isScheduleDisplayDate, isRTL, currentDate, startPosition, padding, resourceEnabled);
  }

  void _pointerHoverEvent(PointerHoverEvent event, bool isScheduleDisplayDate, bool isRTL, [DateTime? currentDate, double? startPosition, double padding = 0, bool resourceEnabled = false]) {
    _updateMouseHoverPosition(event.position, isScheduleDisplayDate, isRTL, currentDate, startPosition, padding, resourceEnabled);
  }
  ...
  DateTime _getMaxAppointmentDate(List<CalendarAppointment> appointments, String? timeZone, DateTime maxDate, DateTime displayDate, ScheduleViewSettings scheduleViewSettings, bool useMobilePlatformUI) {
    ...
    for (int j = 0; j < appointments.length; j++) {
      ...
      appointment.actualEndTime = AppointmentHelper.convertTimeToAppointmentTimeZone(appointment.endTime, appointment.endTimeZone, timeZone);
  ...
  /// and schedule view settings.
  DateTime _getMinAppointmentDate(List<CalendarAppointment> appointments, String? timeZone, DateTime minDate, DateTime displayDate, ScheduleViewSettings scheduleViewSettings, bool useMobilePlatformUI) {
    ...
    for (int j = 0; j < appointments.length; j++) {
      final CalendarAppointment appointment = appointments[j];
      appointment.actualStartTime = AppointmentHelper.convertTimeToAppointmentTimeZone(appointment.startTime, appointment.startTimeZone, timeZone);
  ...
  bool _isAppointmentBetweenDates(List<CalendarAppointment> appointments, DateTime startDate, DateTime endDate, String? timeZone) {
    ...
    for (int j = 0; j < appointments.length; j++) {
      ...
      final List<DateTime> recursiveDates = RecurrenceHelper.getRecurrenceDateTimeCollection(rule, appointment.actualStartTime, recurrenceDuration: AppointmentHelper.getDifference(appointment.actualStartTime, appointment.actualEndTime), specificStartDate: startDate, specificEndDate: endDate);
  ...
  Map<DateTime, List<CalendarAppointment>> _getAppointmentCollectionOnDateBasis(List<CalendarAppointment> appointmentCollection, DateTime startDate, DateTime endDate) {
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL) {
    ...
    if (isLoadMore) {
      ...
      _minDate ??= _getMinAppointmentDate(_appointments, widget.timeZone, widget.minDate, scheduleDisplayDate, widget.scheduleViewSettings, _useMobilePlatformUI);
      ...
      _maxDate ??= _getMaxAppointmentDate(_appointments, widget.timeZone, widget.maxDate, scheduleCurrentDate, widget.scheduleViewSettings, _useMobilePlatformUI);
        ...
        if (_nextDates.isNotEmpty && index > _nextDates.length - 20) {
          ...
          while (count < 20) {
            for (int i = 1; i <= 100; i++) {
              ...
              if (!hideEmptyAgendaDays || _isAppointmentBetweenDates(_appointments, updateDate, weekEndDate, widget.timeZone) || isDateWithInDateRange(updateDate, weekEndDate, scheduleDisplayDate) || isDateWithInDateRange(updateDate, weekEndDate, scheduleCurrentDate)) {
        ...
        if (_previousDates.isNotEmpty && -index > _previousDates.length - 20) {
          ...
          while (count < 20) {
            for (int i = 1; i <= 100; i++) {
              ...
              /// when [hideEmptyAgendaDays] as enabled and display date and
              /// current date not in between the week.
              if (!hideEmptyAgendaDays || _isAppointmentBetweenDates(_appointments, updatedDate, weekEndDate, widget.timeZone) || isDateWithInDateRange(updatedDate, weekEndDate, scheduleDisplayDate) || isDateWithInDateRange(updatedDate, weekEndDate, scheduleCurrentDate)) {
    ...
    if (((index == 0 && _previousDates.isEmpty) || (index < 0 && -index > _previousDates.length - 1)) && startDate.month == _minDate!.month) {
    ...
    bool isNeedMonthBuilder = _useMobilePlatformUI && (prevEndDate.month != startDate.month || prevEndDate.year != startDate.year || initialMonthHeader);
    ...
    if (isNeedMonthBuilder && widget.scheduleViewSettings.hideEmptyScheduleWeek && startDate.month != endDate.month) {
      ...
      if ((startDate.month != scheduleDisplayDate.month || startDate.year != scheduleDisplayDate.year) && (startDate.month != scheduleCurrentDate.month || startDate.year != scheduleCurrentDate.year) && (firstAppointmentDate == null || (startDate.month != firstAppointmentDate.month || startDate.year != firstAppointmentDate.year))) {
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      double panelHeight = ((eventsCount - allDayEventCount) * appointmentViewHeight) + (allDayEventCount * allDayAppointmentHeight);
    ...
    if (_useMobilePlatformUI) {
      ...
      widgets.add(_getMonthOrWeekHeader(startDate, endDate, isRTL, false, viewPadding: viewPadding, isNeedTopPadding: isNeedMonthBuilder));
    ...
    if (isNeedInBetweenMonthBuilder && (!isNextMonthHasNoAppointment || isSameOrBeforeDate(_maxDate, DateTime(endDate.year, endDate.month)))) {
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      void addMonthHeaderView() {
        ...
        scheduleViewDetails._intersectPoint = currentIndex >= 0 ? previousHeight + interSectPoint + viewTopPadding : previousHeight + height - interSectPoint - viewTopPadding;
      ...
      void addDisplayOrCurrentDateView({bool isDisplayDate = true}) {
        final double highlightViewStartPosition = currentIndex >= 0 ? previousHeight + interSectPoint : -(previousHeight + height - interSectPoint);
        widgets.add(_getDisplayDateView(isRTL, isDisplayDate ? scheduleDisplayDate : scheduleCurrentDate, highlightViewStartPosition, viewPadding, appointmentViewHeaderHeight, padding));
      ...
      /// month differ from the week start date then assign the intersection
      /// point.
      if (scheduleViewDetails._intersectPoint == -1 && (startDate.month != currentDate.month || startDate.year != currentDate.year)) {
        addMonthHeaderView();
      }
      ...
      final double viewStartPosition = currentIndex >= 0 ? previousHeight + interSectPoint : -(previousHeight + height - interSectPoint);
      ...
      widgets.add(MouseRegion(
          ...
          child: GestureDetector(
            child: _ScheduleAppointmentView(
                header: CustomPaint(painter: _AgendaDateTimePainter(currentDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform), size: Size(viewPadding, appointmentViewHeaderHeight)),
                content: Container(
                  padding: EdgeInsets.fromLTRB(isRTL ? 0 : viewPadding, appointmentViewTopPadding, isRTL ? viewPadding : 0, appointmentViewTopPadding),
                  child: AgendaViewLayout(null, widget.scheduleViewSettings, currentDate, currentAppointments, isRTL, _locale, _localizations, _calendarTheme, _agendaViewNotifier, widget.appointmentTimeTextFormat, viewPadding, _textScaleFactor, _isMobilePlatform, widget.appointmentBuilder, _minWidth - viewPadding, panelHeight, widget),
                )),
            onTapUp: (TapUpDetails details) {
              _removeDatePicker();
              if (widget.allowViewNavigation && ((!_isRTL && details.localPosition.dx < viewPadding) || (_isRTL && details.localPosition.dx > _minWidth - viewPadding))) {
    ...
    /// end date month value.
    if (!isDisplayDateHighlightAdded && endDate.month != scheduleDisplayDate.month) {
      final double highlightViewStartPosition = currentIndex >= 0 ? previousHeight + topHeight + appointmentHeight : previousHeight + height - topHeight - appointmentHeight;
      widgets.add(_getDisplayDateView(isRTL, scheduleDisplayDate, highlightViewStartPosition, viewPadding, appointmentViewHeaderHeight, padding));
    ...
    if (!isCurrentDateHighlightAdded && endDate.month != scheduleCurrentDate.month) {
      final double highlightViewStartPosition = currentIndex >= 0 ? previousHeight + topHeight + appointmentHeight : previousHeight + height - topHeight - appointmentHeight;
      widgets.add(_getDisplayDateView(isRTL, scheduleCurrentDate, highlightViewStartPosition, viewPadding, appointmentViewHeaderHeight, padding));
    ...
    /// month different and week does not have appointments or week have
    /// appointments but end date month dates does not have an appointment
    if (_useMobilePlatformUI && isNeedInBetweenMonthBuilder && isNextMonthHasNoAppointment && isSameOrBeforeDate(_maxDate, DateTime(endDate.year, endDate.month))) {
      /// Calculate and assign the intersection point because the current
      /// view holds next month label. if scrolling reaches this position
      /// then we update the header date so add the location to intersecting
      /// point.
      scheduleViewDetails._intersectPoint = currentIndex >= 0 ? previousHeight + topHeight + appointmentHeight + viewTopPadding : previousHeight + height - topHeight - appointmentHeight - viewTopPadding;
  ...
  Widget _getMonthOrWeekHeader(DateTime startDate, DateTime? endDate, bool isRTL, bool isMonthLabel, {double viewPadding = 0, bool isNeedTopPadding = false}) {
    const double padding = 5;
    Widget? headerWidget;
    if (isMonthLabel && widget.scheduleViewMonthHeaderBuilder != null) {
      final ScheduleViewMonthHeaderDetails details = ScheduleViewMonthHeaderDetails(DateTime(startDate.year, startDate.month), Rect.fromLTWH(0, 0, _minWidth, widget.scheduleViewSettings.monthHeaderSettings.height));
      headerWidget = widget.scheduleViewMonthHeaderBuilder!(context, details);
    }

    return GestureDetector(
        child: Container(
            padding: isMonthLabel ? EdgeInsets.fromLTRB(0, isNeedTopPadding ? padding : 0, 0, 0) : EdgeInsets.fromLTRB(isRTL ? 0 : viewPadding, isNeedTopPadding ? padding : 0, isRTL ? viewPadding : 0, 0),
            child: RepaintBoundary(
                child: headerWidget != null
                    ...
                    : CustomPaint(
                        painter: _ScheduleLabelPainter(startDate, endDate, widget.scheduleViewSettings, isMonthLabel, isRTL, _locale, _useMobilePlatformUI, _agendaViewNotifier, _calendarTheme, _localizations, _textScaleFactor),
                        size: isMonthLabel ? Size(_minWidth, widget.scheduleViewSettings.monthHeaderSettings.height) : Size(_minWidth - viewPadding - (2 * padding), widget.scheduleViewSettings.weekHeaderSettings.height),
                      ))),
        onTapUp: (TapUpDetails details) {
          _removeDatePicker();
          if (!CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.onTap)) {
          ...
          CalendarViewHelper.raiseCalendarTapCallback(widget, DateTime(startDate.year, startDate.month, startDate.day), null, isMonthLabel ? CalendarElement.header : CalendarElement.viewHeader, null);
        },
        onLongPressStart: (LongPressStartDetails details) {
          _removeDatePicker();
          if (!CalendarViewHelper.shouldRaiseCalendarLongPressCallback(widget.onLongPress)) {
          ...
          CalendarViewHelper.raiseCalendarLongPressCallback(widget, DateTime(startDate.year, startDate.month, startDate.day), null, isMonthLabel ? CalendarElement.header : CalendarElement.viewHeader, null);
        });
  }

  Widget _getDisplayDateView(bool isRTL, DateTime currentDisplayDate, double highlightViewStartPosition, double viewHeaderWidth, double displayDateHighlightHeight, double padding) {
    return MouseRegion(
        onEnter: (PointerEnterEvent event) {
        ...
        child: GestureDetector(
          child: _ScheduleAppointmentView(
              header: CustomPaint(painter: _AgendaDateTimePainter(currentDisplayDate, null, widget.scheduleViewSettings, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform), size: Size(viewHeaderWidth, displayDateHighlightHeight)),
              content: Container(
                padding: EdgeInsets.fromLTRB(isRTL ? 0 : viewHeaderWidth, 0, isRTL ? viewHeaderWidth : 0, 0),
                child: CustomPaint(painter: _ScheduleLabelPainter(currentDisplayDate, null, widget.scheduleViewSettings, false, isRTL, _locale, _useMobilePlatformUI, _agendaViewNotifier, _calendarTheme, _localizations, _textScaleFactor, isDisplayDate: true), size: Size(_minWidth - viewHeaderWidth, displayDateHighlightHeight)),
              )),
          onTapUp: (TapUpDetails details) {
            _removeDatePicker();
            if (widget.allowViewNavigation && ((!_isRTL && details.localPosition.dx < viewHeaderWidth) || (_isRTL && details.localPosition.dx > _minWidth - viewHeaderWidth))) {
              _controller.view = CalendarView.day;
            ...
            _raiseCallbackForScheduleView(currentDisplayDate, details.localPosition, <CalendarAppointment>[], viewHeaderWidth, padding, true, isDisplayDate: true);
          },
          onLongPressStart: (LongPressStartDetails details) {
            _removeDatePicker();
            if (widget.allowViewNavigation && ((!_isRTL && details.localPosition.dx < viewHeaderWidth) || (_isRTL && details.localPosition.dx > _minWidth - viewHeaderWidth))) {
            ...
            _raiseCallbackForScheduleView(currentDisplayDate, details.localPosition, <CalendarAppointment>[], viewHeaderWidth, padding, false, isDisplayDate: true);
          },
        ));
  }

  void _raiseCallbackForScheduleView(DateTime currentDate, Offset offset, List<CalendarAppointment> appointments, double viewHeaderWidth, double padding, bool isTapCallback, {bool isDisplayDate = false}) {
    ...
    if ((!_isRTL && viewHeaderWidth >= offset.dx) || (_isRTL && _minWidth - viewHeaderWidth < offset.dx)) {
      ...
      if (isDisplayDate) {
        if (isTapCallback) {
          CalendarViewHelper.raiseCalendarTapCallback(widget, DateTime(currentDate.year, currentDate.month, currentDate.day), null, CalendarElement.calendarCell, null);
        } else {
          CalendarViewHelper.raiseCalendarLongPressCallback(widget, DateTime(currentDate.year, currentDate.month, currentDate.day), null, CalendarElement.calendarCell, null);
        }
      ...
      for (int k = 0; k < appointments.length; k++) {
        ...
        final double currentAppointmentHeight = (_useMobilePlatformUI && _isAllDayAppointmentView(appointment) ? allDayItemHeight : itemHeight) + padding;
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    final DateTime scheduleDisplayDate = DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, _scheduleDisplayDate));
    ...
    /// in [ScheduleViewSettings] disabled else it return min
    /// start date of the appointment collection.
    _minDate = _getMinAppointmentDate(_appointments, widget.timeZone, widget.minDate, currentMinDate, widget.scheduleViewSettings, _useMobilePlatformUI);
    ...
    /// in [ScheduleViewSettings] disabled else it return max
    /// end date of the appointment collection.
    _maxDate = _getMaxAppointmentDate(_appointments, widget.timeZone, widget.maxDate, currentMaxDate, widget.scheduleViewSettings, _useMobilePlatformUI);
    ...
    if (_previousDates.isEmpty) {
      ...
      /// collection as empty.
      DateTime date = _nextDates.isNotEmpty ? _nextDates[0] : DateTimeHelper.getDateTimeValue(addDays(scheduleDisplayDate, value));
      int count = 0;
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(viewStartDate, isSameOrBeforeDate(_maxDate, viewEndDate) ? viewEndDate : _maxDate!, _appointments, widget.timeZone, false);

      const double padding = 5;
      Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(appointmentCollection, viewStartDate, viewEndDate);
      ...
      while (totalHeight < height && (_previousDates.isNotEmpty || totalHeight == 0)) {
        ...
        if (totalHeight != 0) {
          ...
          appointmentCollection = AppointmentHelper.getVisibleAppointments(viewStartDate, isSameOrBeforeDate(_maxDate, viewEndDate) ? viewEndDate : _maxDate!, _appointments, widget.timeZone, false);
        ...
        totalHeight = ((numberOfEvents + 1) * padding) + ((numberOfEvents - allDayCount) * appointmentViewHeight) + (allDayCount * allDayAppointmentHeight) + labelHeight;
      }
      ...
      if (_nextDates.isNotEmpty && isNewDatesAdded) {
        _headerUpdateNotifier.value = DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, _nextDates[0]));
      }
    }
    ...
    if (_nextDates.isNotEmpty && _agendaScrollController!.initialScrollOffset == 0 && !_agendaScrollController!.hasClients) {
      ...
      if (viewStartDate.isBefore(scheduleDisplayDate) && !isSameDate(viewStartDate, scheduleDisplayDate) && isSameOrBeforeDate(viewEndDate, scheduleDisplayDate)) {
        final DateTime viewEndDate = DateTimeHelper.getDateTimeValue(addDays(scheduleDisplayDate, -1));

        final double initialScrollPosition = _getInitialScrollPosition(viewStartDate, viewEndDate, scheduleCurrentDate, appointmentViewHeight, allDayAppointmentHeight);
        if (initialScrollPosition != 0) {
          _agendaScrollController?.removeListener(_handleScheduleViewScrolled);
          _agendaScrollController = ScrollController(initialScrollOffset: initialScrollPosition)..addListener(_handleScheduleViewScrolled);
        }
        ...
        while (visibleStartDate.isBefore(scheduleDisplayDate) && !isSameDate(visibleStartDate, scheduleDisplayDate)) {
          ...
          initialScrollPosition += _getInitialScrollPosition(appStartDate, appEndDate, scheduleCurrentDate, appointmentViewHeight, allDayAppointmentHeight);
  ...
  double _getInitialScrollPosition(DateTime viewStartDate, DateTime viewEndDate, DateTime scheduleCurrentDate, double appointmentViewHeight, double allDayAppointmentHeight) {
    ...
    final List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(viewStartDate, viewEndDate, _appointments, widget.timeZone, false);
    ...
    if (viewStartDate.isBefore(scheduleCurrentDate) && !isSameDate(viewStartDate, scheduleCurrentDate) && isSameOrBeforeDate(viewEndDate, scheduleCurrentDate)) {
    ...
    if (appointmentCollection.isNotEmpty) {
      final Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(appointmentCollection, viewStartDate, viewEndDate);
      final List<DateTime> dateAppointmentKeys = dateAppointments.keys.toList();
      ...
      initialScrolledPosition = todayNewEventHeight +
          ...
          (viewStartDate.month == _scheduleDisplayDate.month && viewStartDate.day != 1 ? 0 : (!_useMobilePlatformUI ? 0 : widget.scheduleViewSettings.monthHeaderSettings.height + padding));
    } else if ((viewStartDate.month != _scheduleDisplayDate.month && _useMobilePlatformUI) || todayNewEventHeight != 0) {
      initialScrolledPosition = (!_useMobilePlatformUI ? 0 : widget.scheduleViewSettings.weekHeaderSettings.height + padding) + todayNewEventHeight;
    }
  ...
  Widget addAgendaWithLoadMore(double height, bool isRTL) {
    ...
    if (_previousDates.isEmpty || !isSameDate(_previousDates[_previousDates.length - 1], viewMinDate)) {
      /// Calculate the start date from display date if next view dates
      /// collection as empty.
      DateTime date = _previousDates.isNotEmpty ? _previousDates[_previousDates.length - 1] : (_nextDates.isNotEmpty ? _nextDates[0] : DateTimeHelper.getDateTimeValue(addDays(scheduleDisplayDate, value)));
      int count = 0;
    ...
    final DateTime viewMaxDate = DateTimeHelper.getDateTimeValue(addDays(_maxDate, (DateTime.daysPerWeek - _maxDate!.weekday) % DateTime.daysPerWeek));
    if (_nextDates.isEmpty || !isSameDate(_nextDates[_nextDates.length - 1], viewMaxDate)) {
      /// Calculate the start date from display date
      DateTime date = _nextDates.isEmpty ? DateTimeHelper.getDateTimeValue(addDays(scheduleDisplayDate, value)) : DateTimeHelper.getDateTimeValue(addDays(_nextDates[_nextDates.length - 1], DateTime.daysPerWeek));
      int count = 0;
    ...
    /// in between space between the May 23 to May 28 and assign the value to
    /// scroll controller initial scroll position
    if (_nextDates.isNotEmpty && _agendaScrollController!.initialScrollOffset == 0 && !isInitialLoadMore && !_isNeedLoadMore && !_isScheduleStartLoadMore && isSameDate(visibleMaxDate, _scheduleMaxDate) && isSameDate(visibleMinDate, _scheduleMinDate)) {
      DateTime viewStartDate = _nextDates[0];
      const double padding = 5;
      final double appointmentViewHeight = CalendarViewHelper.getScheduleAppointmentHeight(null, widget.scheduleViewSettings);
      final double allDayAppointmentHeight = CalendarViewHelper.getScheduleAllDayAppointmentHeight(null, widget.scheduleViewSettings);
      ...
      final double displayEventHeight = _useMobilePlatformUI ? appointmentViewHeaderHeight : appointmentViewHeaderHeight + dividerHeight;
      ...
      while (isSameOrBeforeDate(_maxDate, viewStartDate)) {
        ...
        double todayNewEventHeight = isDateWithInDateRange(viewStartDate, viewEndDate, scheduleCurrentDate) ? displayEventHeight : 0;

        /// Display date view height.
        double displayNewEventHeight = isDateWithInDateRange(viewStartDate, viewEndDate, scheduleDisplayDate) ? displayEventHeight : 0;

        /// Current week appointments heights.
        final List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(appStartDate, appEndDate, _appointments, widget.timeZone, false, canCreateNewAppointment: false);

        /// Check the week date needs month header or not.
        final bool isNeedMonthBuilder = _useMobilePlatformUI && ((viewStartDate.month != appEndDate.month || viewStartDate.year != appEndDate.year) || viewStartDate.day == 1);
        ...
        if (appointmentCollection.isNotEmpty) {
          /// Get the collection of appointment collection listed by date.
          final Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(appointmentCollection, appStartDate, appEndDate);
          final List<DateTime> dateAppointmentKeys = dateAppointments.keys.toList();
      ...
      while (viewStartDate.isBefore(scheduleDisplayDate) && !isSameDate(viewStartDate, scheduleDisplayDate)) {
        ...
        /// Today date view height.
        double todayNewEventHeight = !isSameDate(scheduleCurrentDate, scheduleDisplayDate) && isDateWithInDateRange(appStartDate, appEndDate, scheduleCurrentDate) ? displayEventHeight : 0;
        final List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(appStartDate, appEndDate, _appointments, widget.timeZone, false, canCreateNewAppointment: false);
        ...
        if (appointmentCollection.isNotEmpty) {
          ...
          /// Web view does not have month label.
          initialScrolledPosition += isNeedMonthBuilder ? widget.scheduleViewSettings.monthHeaderSettings.height + padding : 0;
      ...
      if (initialScrolledPosition != 0) {
        ...
        _agendaScrollController?.removeListener(_handleScheduleViewScrolled);
        _agendaScrollController = ScrollController(initialScrollOffset: initialScrolledPosition)..addListener(_handleScheduleViewScrolled);
        _scrollKey = UniqueKey();
      }
    }
          ...
          child: _OpacityWidget(
              ...
              child: NotificationListener<OverscrollNotification>(
                  onNotification: (OverscrollNotification notification) {
                    ...
                    if (notification.overscroll < 0 && _agendaScrollController!.position.pixels <= _agendaScrollController!.position.minScrollExtent) {
                      DateTime date = AppointmentHelper.getMonthStartDate(DateTime(_scheduleMinDate!.year, _scheduleMinDate!.month - 1));
                  ...
                  child: CustomScrollView(
                    key: _scrollKey,
                    physics: const AlwaysScrollableScrollPhysics(parent: ClampingScrollPhysics(parent: RangeMaintainingScrollPhysics())),
                    controller: _agendaScrollController,
                    center: _scheduleViewKey,
                    slivers: <Widget>[
                      SliverList(
    ...
    if ((_isNeedLoadMore || _isScheduleStartLoadMore) && widget.loadMoreWidgetBuilder != null) {
      final Alignment loadMoreAlignment = _agendaScrollController!.hasClients && _agendaScrollController!.position.pixels <= _agendaScrollController!.position.minScrollExtent && _isScheduleStartLoadMore ? Alignment.topCenter : Alignment.bottomCenter;
      final DateTime visibleStartDate = _isNeedLoadMore ? AppointmentHelper.getMonthStartDate(_scheduleMaxDate!) : _scheduleMinDate!;
  ...
  Widget _getCalendarViewPopup() {
    ...
    final List<Widget> children = <Widget>[];
    double width = 0;
    Color? headerTextColor = widget.headerStyle.textStyle != null ? widget.headerStyle.textStyle!.color : (_calendarTheme.headerTextStyle!.color);
    headerTextColor ??= Colors.black87;
    final TextStyle style = TextStyle(color: headerTextColor, fontSize: 12);
    int selectedIndex = -1;
    final Color? todayColor = CalendarViewHelper.getTodayHighlightTextColor(widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _calendarTheme);
    ...
    double? headerIconTextWidth = widget.headerStyle.textStyle != null ? widget.headerStyle.textStyle!.fontSize : _calendarTheme.headerTextStyle!.fontSize;
    headerIconTextWidth ??= 14;
    final double totalArrowWidth = 2 * arrowWidth;
    final bool isCenterAlignment = !_isMobilePlatform && (widget.headerStyle.textAlign == TextAlign.center || widget.headerStyle.textAlign == TextAlign.justify);

    /// Calculate the calendar view button width that placed on header view
    final double calendarViewWidth = _useMobilePlatformUI ? iconWidth : _getTextWidgetWidth(calendarViews[_view]!, widget.headerHeight, _minWidth - totalArrowWidth, context, style: style).width + padding + headerIconTextWidth;
    double dividerWidth = 0;
    double todayWidth = 0;

    /// Today button shown only the date picker enabled.
    if (widget.showDatePickerButton) {
      todayWidth = _useMobilePlatformUI ? iconWidth : _getTextWidgetWidth(_localizations.todayLabel, widget.headerHeight, _minWidth - totalArrowWidth, context, style: style).width + padding;
    ...
    if (_isMobilePlatform) {
      ...
      if (widget.headerStyle.textAlign == TextAlign.right || widget.headerStyle.textAlign == TextAlign.end) {
        ...
        final double leftStartPosition = (_minWidth - totalArrowWidth - calendarViewWidth - dividerWidth - todayWidth - headerWidth) / 2;
        left = _isRTL ? leftStartPosition + calendarViewWidth - width : leftStartPosition + totalArrowWidth + headerWidth + todayWidth + dividerWidth;
      }
    }
  ...
  Widget _addResourcePanel(bool isResourceEnabled, double resourceViewSize, double height, bool isRTL) {
    ...
    final double timeLabelSize = CalendarViewHelper.getTimeLabelWidth(widget.timeSlotViewSettings.timeRulerSize, _view);
    final double top = viewHeaderHeight + timeLabelSize;
    final double resourceItemHeight = CalendarViewHelper.getResourceItemHeight(resourceViewSize, height - top, widget.resourceViewSettings, _resourceCollection!.length);
              ...
              child: MouseRegion(
                  ...
                  child: GestureDetector(
                    child: ScrollConfiguration(
                      ...
                      child: ListView(padding: EdgeInsets.zero, physics: const ClampingScrollPhysics(), controller: _resourcePanelScrollController, children: <Widget>[
                        ResourceViewWidget(_resourceCollection, widget.resourceViewSettings, resourceItemHeight, widget.cellBorderColor, _calendarTheme, _resourceImageNotifier, isRTL, _textScaleFactor, _resourceHoverNotifier.value, _imagePainterCollection, resourceViewSize, panelHeight, widget.resourceViewHeaderBuilder),
                      ]),
                    ),
                    onTapUp: (TapUpDetails details) {
  ...
  void _handleOnLongPressForResourcePanel(LongPressStartDetails details, double resourceItemHeight) {
    ...
    CalendarViewHelper.raiseCalendarLongPressCallback(widget, null, resourceAppointments, CalendarElement.resourceHeader, tappedResource);
  }
  ...
  void _handleOnTapForResourcePanel(TapUpDetails details, double resourceItemHeight) {
    ...
    final CalendarResource tappedResource = _getTappedResource(details.localPosition.dy, resourceItemHeight);
    final List<dynamic> resourceAppointments = _getSelectedResourceAppointments(tappedResource);
    CalendarViewHelper.raiseCalendarTapCallback(widget, null, resourceAppointments, CalendarElement.resourceHeader, tappedResource);
  }
  ...
  Widget _addCustomScrollView(double top, double resourceViewSize, bool isRTL, bool isResourceEnabled, double width, double height, double agendaHeight) {
    return Positioned(
      top: top,
  ...
  Widget _addChildren(double agendaHeight, double height, double width, bool isRTL) {
    ...
    final List<Widget> children = <Widget>[
      ...
      _addCustomScrollView(widget.headerHeight, resourceViewSize, isRTL, isResourceEnabled, width, height, agendaHeight),
    ...
    if (_isNeedLoadMore && widget.loadMoreWidgetBuilder != null) {
      children.add(Container(
          color: Colors.transparent,
          child: widget.loadMoreWidgetBuilder!(context, () async {
            await loadMoreAppointments(_currentViewVisibleDates[0], _currentViewVisibleDates[_currentViewVisibleDates.length - 1]);
          })));
    }
  ...
  Widget _addDatePicker(double top, bool isRTL) {
    ...
    final Color? todayTextColor = CalendarViewHelper.getTodayHighlightTextColor(todayColor, widget.todayTextStyle, _calendarTheme);
    double left = 0;
    if (_isMobilePlatform) {
      ...
      if (widget.headerStyle.textAlign == TextAlign.right || widget.headerStyle.textAlign == TextAlign.end) {
        ...
        if (headerViewWidth == _minWidth) {
          ...
          left = _isRTL ? leftPadding + arrowWidth + calendarViewWidth + headerViewWidth - containerSize : leftPadding + arrowWidth + headerPadding;
        }
      }
    }
        ...
        child: _PopupWidget(
            child: Container(
                ...
                child: SfDateRangePicker(
                  ...
                  monthCellStyle: DateRangePickerMonthCellStyle(textStyle: datePickerStyle, todayTextStyle: datePickerStyle.copyWith(color: todayTextColor)),
                  yearCellStyle: DateRangePickerYearCellStyle(
                    textStyle: datePickerStyle,
                    todayTextStyle: datePickerStyle.copyWith(color: todayTextColor),
                    leadingDatesTextStyle: widget.monthViewSettings.monthCellStyle.leadingDatesTextStyle ?? _calendarTheme.leadingDatesTextStyle,
                  ),
                  view: _view == CalendarView.month || _view == CalendarView.timelineMonth ? DateRangePickerView.year : DateRangePickerView.month,
                  onViewChanged: (DateRangePickerViewChangedArgs details) {
                    if ((_view != CalendarView.month && _view != CalendarView.timelineMonth) || details.view != DateRangePickerView.month) {
                      return;
                    }
                    ...
                    _controller.displayDate = DateTime(selectedDate.year, selectedDate.month, selectedDate.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
                  },
                  onSelectionChanged: (DateRangePickerSelectionChangedArgs details) {
                    ...
                    _showHeader = false;
                    _controller.displayDate = DateTime(details.value.year, details.value.month, details.value.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
                  },
                ))));
  }
  ...
  void _updateCalendarState(UpdateCalendarStateDetails details) {
    ...
    if (_currentViewVisibleDates != details.currentViewVisibleDates) {
      ...
      _updateVisibleAppointments();
      if (CalendarViewHelper.shouldRaiseViewChangedCallback(widget.onViewChanged)) {
        final bool showTrailingLeadingDates = CalendarViewHelper.isLeadingAndTrailingDatesVisible(widget.monthViewSettings.numberOfWeeksInView, widget.monthViewSettings.showTrailingAndLeadingDates);
  ...
  void _handleTapForAgenda(TapUpDetails details, DateTime? selectedDate) {
    _removeDatePicker();
    if (widget.allowViewNavigation && ((!_isRTL && details.localPosition.dx < _agendaDateViewWidth) || (_isRTL && details.localPosition.dx > _minWidth - _agendaDateViewWidth))) {
      _controller.view = CalendarView.day;
    ...
    CalendarViewHelper.raiseCalendarTapCallback(widget, selectedDate, selectedAppointments, selectedAppointments.isNotEmpty ? CalendarElement.appointment : CalendarElement.agenda, null);
  }
  ...
  void _handleLongPressForAgenda(LongPressStartDetails details, DateTime? selectedDate) {
    ...
    CalendarViewHelper.raiseCalendarLongPressCallback(widget, selectedDate, selectedAppointments, selectedAppointments.isNotEmpty ? CalendarElement.appointment : CalendarElement.agenda, null);
  }
  ...
  List<dynamic> _getSelectedAppointments(Offset localPosition, DateTime? selectedDate) {
    ...
    if ((!_isRTL && localPosition.dx < _agendaDateViewWidth) || (_isRTL && localPosition.dx > _minWidth - _agendaDateViewWidth)) {
      return <dynamic>[];
    }

    List<CalendarAppointment> agendaAppointments = AppointmentHelper.getSelectedDateAppointments(_appointments, widget.timeZone, selectedDate);
    ...
    agendaAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => app1.actualStartTime.compareTo(app2.actualStartTime));
    agendaAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
    agendaAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isSpanned, app2.isSpanned));
    ...
    for (int i = 0; i < agendaAppointments.length; i++) {
      ...
      final double appointmentHeight = _isAllDayAppointmentView(appointment) ? allDayAppointmentHeight : actualAppointmentHeight;
  ...
  Widget _addAgendaView(double height, double startPosition, double width, bool isRTL) {
      ...
      return Positioned(
          ...
          child: _OpacityWidget(
              ...
              child: Container(
                  ...
                  child: GestureDetector(
                    child: AgendaViewLayout(widget.monthViewSettings, null, currentSelectedDate, null, isRTL, _locale, _localizations, _calendarTheme, _agendaViewNotifier, widget.appointmentTimeTextFormat, 0, _textScaleFactor, _isMobilePlatform, widget.appointmentBuilder, width, height, widget),
                    onTapUp: (TapUpDetails details) {
    ...
    final List<CalendarAppointment> agendaAppointments = AppointmentHelper.getSelectedDateAppointments(_appointments, widget.timeZone, currentSelectedDate);
    agendaAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => app1.actualStartTime.compareTo(app2.actualStartTime));
    agendaAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
    agendaAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isSpanned, app2.isSpanned));
    ...
    if (agendaAppointments.isNotEmpty) {
      ...
      painterHeight = ((count * (allDayAppointmentHeight + topPadding)) + ((agendaAppointments.length - count) * (appointmentHeight + topPadding))) + bottomPadding;
    }
            ...
            child: Container(
                ...
                child: MouseRegion(
                    ...
                    child: GestureDetector(
                      child: Stack(children: <Widget>[
                        CustomPaint(
                          painter: _AgendaDateTimePainter(currentSelectedDate, widget.monthViewSettings, null, widget.todayHighlightColor ?? _calendarTheme.todayHighlightColor, widget.todayTextStyle, _locale, _calendarTheme, _agendaDateNotifier, _minWidth, isRTL, _textScaleFactor, _isMobilePlatform),
...
class _CalendarHeaderViewState extends State<_CalendarHeaderView> {
  ...
  Widget build(BuildContext context) {
    ...
    final String rawHeaderString = _getHeaderText();
    final String headerString = rawHeaderString[0].toUpperCase() + rawHeaderString.substring(1).toLowerCase();
    ...
    final List<Widget> children = <Widget>[];
    Color? headerTextColor = widget.headerStyle.textStyle != null ? widget.headerStyle.textStyle!.color : (widget.calendarTheme.headerTextStyle!.color);
    ...
    const double padding = 5;
    double? headerIconTextWidth = widget.headerStyle.textStyle != null ? widget.headerStyle.textStyle!.fontSize : widget.calendarTheme.headerTextStyle!.fontSize;
    headerIconTextWidth ??= 14;
    ...
    if (widget.showDatePickerButton) {
      ...
      if (!useMobilePlatformUI) {
        ...
        final Size todayButtonSize = _getTextWidgetWidth(todayText, widget.height, widget.width - totalArrowWidth, context, style: const TextStyle(fontSize: defaultCalendarViewTextSize));
    ...
    final TextStyle weekNumberTextStyle = widget.weekNumberStyle.textStyle ?? widget.calendarTheme.weekNumberTextStyle!;

    final Color? weekNumberBackgroundColor = widget.weekNumberStyle.backgroundColor ?? widget.calendarTheme.weekNumberBackgroundColor;
    ...
    if (!widget.isMobilePlatform || (widget.isMobilePlatform && weekNumberEnabled)) {
      final Size headerTextSize = _getTextWidgetWidth(headerString, widget.height, widget.width - totalArrowWidth - todayIconWidth - padding, context, style: widget.headerStyle.textStyle ?? widget.calendarTheme.headerTextStyle);
    ...
    if (weekNumberEnabled) {
      final Size weekNumberPanelSize = _getTextWidgetWidth('${widget.localizations.weeknumberLabel}$weekNumberString ', widget.height, widget.width, context, style: weekNumberTextStyle);
    ...
    final Color? highlightColor = CalendarViewHelper.getTodayHighlightTextColor(widget.todayHighlightColor ?? widget.calendarTheme.todayHighlightColor, widget.todayTextStyle, widget.calendarTheme);
    ...
    if (isNeedViewSwitchOption) {
      ...
      if (useMobilePlatformUI) {
        ...
        final double totalWidth = widget.width - totalArrowWidth - dividerWidth - todayIconWidth - headerTextWidth - weekNumberPanelWidth;
        ...
        for (int i = 0; i < allowedViewsLength; i++) {
          ...
          final Size calendarViewSize = _getTextWidgetWidth(_calendarViews[currentView]!, widget.height, totalWidth, context, style: const TextStyle(fontSize: defaultCalendarViewTextSize));
          final double currentViewTextWidth = calendarViewSize.width + allowedViewsPadding;
          maxCalendarViewHeight = maxCalendarViewHeight > calendarViewSize.height ? maxCalendarViewHeight : calendarViewSize.height + allowedViewsPadding;
        ...
        if (allowedViewsWidth < totalWidth) {
          ...
          for (int i = 0; i < allowedViewsLength; i++) {
            final CalendarView currentView = widget.allowedViews![i];
            children.add(_getCalendarViewWidget(headerBackgroundColor, useMobilePlatformUI, false, calendarViewsWidth[currentView]!, maxHeaderHeight, style, arrowColor, headerTextColor, currentView, widget.view == currentView, highlightColor, defaultCalendarViewTextSize));
          }
        } else {
          /// Render allowed views drop down when header view does not have a
          /// space to hold the allowed views.
          final Size calendarViewSize = _getTextWidgetWidth(_calendarViews[widget.view]!, widget.height, widget.width - totalArrowWidth, context, style: const TextStyle(fontSize: defaultCalendarViewTextSize));
          ...
          children.add(_getCalendarViewWidget(headerBackgroundColor, useMobilePlatformUI, true, calendarViewWidth, maxHeaderHeight, style, arrowColor, headerTextColor, widget.view, widget.viewChangeNotifier.value, highlightColor, defaultCalendarViewTextSize, semanticLabel: 'CalendarView'));
        }
      }
    }
    ...
    if (weekNumberEnabled) {
      ...
      final double remainingWidth = widget.width - calendarViewWidth - todayIconWidth - dividerWidth - totalArrowWidth - weekNumberPanelWidth - headerWidth;
      if (remainingWidth < 0) {
    ...
    if (!DateTimeHelper.canMoveToNextView(widget.view, widget.numberOfWeeksInView, widget.minDate, widget.maxDate, dates, widget.nonWorkingDays)) {
      nextArrowColor = nextArrowColor.withOpacity(nextArrowColor.opacity * 0.5);
    }

    if (!DateTimeHelper.canMoveToPreviousView(widget.view, widget.numberOfWeeksInView, widget.minDate, widget.maxDate, dates, widget.nonWorkingDays)) {
    ...
    final bool isCenterAlignment = !widget.isMobilePlatform && (navigationArrowEnabled || isNeedViewSwitchOption) && (widget.headerStyle.textAlign == TextAlign.center || widget.headerStyle.textAlign == TextAlign.justify);
                  ...
                  child: Container(
                      ...
                      child: Row(
                          ...
                          children: widget.showDatePickerButton
                              ...
                              : <Widget>[Flexible(child: Text(headerString, style: headerTextStyle, maxLines: 1, overflow: TextOverflow.clip, softWrap: false, textDirection: TextDirection.ltr))])),
                )),
          )
        : Container(
            alignment: getHeaderAlignment(),
            ...
            child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                      clipBehavior: Clip.antiAlias,
                      decoration: BoxDecoration(
                        color: widget.showDatePickerButton && widget.isPickerShown ? Colors.grey.withOpacity(0.3) : headerBackgroundColor,
                      ),
                      ...
                      child: Row(
                        ...
                        children: widget.showDatePickerButton
                            ...
                            : <Widget>[
                                Flexible(child: Text(headerString, style: headerTextStyle, maxLines: 1, overflow: TextOverflow.clip, softWrap: false, textDirection: TextDirection.ltr)),
                              ],
                      )),
                )),
          );
    ...
    final Widget weekNumberWidget = weekNumberEnabled
        ? Container(
            ...
            alignment: getHeaderAlignment(),
            child: Container(
                decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(padding)), color: weekNumberBackgroundColor),
                alignment: Alignment.center,
                ...
                child: InkWell(
                  ...
                  child: Semantics(
                    ...
                    child: Container(
                        ...
                        child: Icon(
                          widget.navigationDirection == MonthNavigationDirection.horizontal ? Icons.chevron_left : Icons.keyboard_arrow_up,
                ...
                child: InkWell(
                  ...
                  child: Semantics(
                    ...
                    child: Container(
                        ...
                        child: Icon(
                          widget.navigationDirection == MonthNavigationDirection.horizontal ? Icons.chevron_right : Icons.keyboard_arrow_down,
                ...
                child: InkWell(
                  ...
                  child: Semantics(
                    ...
                    child: useMobilePlatformUI
                        ...
                        : Container(
                            decoration: BoxDecoration(
                              border: Border.all(color: widget.cellBorderColor ?? widget.calendarTheme.cellBorderColor!),
    ...
    final Widget dividerWidget = widget.showDatePickerButton && isNeedViewSwitchOption && !useMobilePlatformUI
        ? Container(
            alignment: Alignment.center,
            color: headerBackgroundColor,
            width: dividerWidth,
  ...
  Widget _getCalendarViewWidget(Color headerBackgroundColor, bool useMobilePlatformUI, bool isNeedIcon, double width, double height, TextStyle style, Color arrowColor, Color headerTextColor, CalendarView view, bool isHighlighted, Color? highlightColor, double defaultCalendarViewTextSize, {String? semanticLabel}) {
    ...
    final Color? allowedViewsColor = isHighlighted ? highlightColor : widget.cellBorderColor ?? widget.calendarTheme.cellBorderColor;
    return Container(
      alignment: Alignment.center,
      color: headerBackgroundColor,
      width: width,
      ...
      padding: EdgeInsets.all(useMobilePlatformUI ? 2 : 4),
      child: Material(
        color: isHighlighted && (isNeedIcon || useMobilePlatformUI) ? Colors.grey.withOpacity(0.3) : headerBackgroundColor,
        child: InkWell(
                ...
                child: useMobilePlatformUI
                    ...
                    : (isNeedIcon
                        ? Container(
                            ...
                            alignment: Alignment.center,
                            clipBehavior: Clip.antiAlias,
                            decoration: BoxDecoration(border: Border.all(color: highlightColor!), borderRadius: BorderRadius.circular(5.0)),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: <Widget>[
                                Text(
                                ...
                                Icon(
                                  ...
                                  color: arrowColor,
                                  size: (widget.headerStyle.textStyle ?? widget.calendarTheme.headerTextStyle)!.fontSize ?? 14,
                                )
                              ],
                            ))
                        : Container(
                            decoration: BoxDecoration(border: Border.all(color: allowedViewsColor!), borderRadius: BorderRadius.circular(5.0)),
                            width: width,
                            height: height,
                            alignment: Alignment.center,
                            child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: isHighlighted ? highlightColor : headerTextColor, fontSize: defaultCalendarViewTextSize), maxLines: 1, textDirection: TextDirection.ltr),
                          )))),
      ),
    );
  }
  ...
  String _getHeaderText() {
    ...
    final String? headerDateFormat = widget.headerDateFormat != null && widget.headerDateFormat!.isNotEmpty ? widget.headerDateFormat : null;
    final int visibleDatesCount = DateTimeHelper.getViewDatesCount(widget.view, widget.numberOfDaysInView, widget.numberOfDaysInView, widget.nonWorkingDays);
    switch (widget.view) {
      case CalendarView.schedule:
        {
...
class _ScheduleLabelPainter extends CustomPainter {
  ...
  void _addDisplayDateLabel(Canvas canvas, Size size) {
    ...
    final TextSpan span = TextSpan(
      ...
      style: scheduleViewSettings.weekHeaderSettings.weekTextStyle ?? const TextStyle(color: Colors.grey, fontSize: 15, fontFamily: 'Roboto'),
    );
    ...
    if (isDisplayDate && agendaViewNotifier.value != null && isSameDate(agendaViewNotifier.value!.hoveringDate, startDate)) {
      const double padding = 5;
      if (useMobilePlatformUI) {
        ...
        final Rect rect = Rect.fromLTWH(0, padding + viewPadding, size.width - (isRTL ? viewPadding : padding), size.height - (2 * (viewPadding + padding)));
  ...
  void _addWeekLabel(Canvas canvas, Size size) {
    ...
    if (scheduleViewSettings.weekHeaderSettings.textAlign == TextAlign.right || scheduleViewSettings.weekHeaderSettings.textAlign == TextAlign.end) {
    ...
    if (isRTL) {
      xPosition = size.width - _textPainter.width - xPosition;
      if (scheduleViewSettings.weekHeaderSettings.textAlign == TextAlign.left || scheduleViewSettings.weekHeaderSettings.textAlign == TextAlign.end) {
        xPosition = 0;
      } else if (scheduleViewSettings.weekHeaderSettings.textAlign == TextAlign.center) {
        xPosition = size.width / 2 - _textPainter.width / 2;
      }
    ...
    _textPainter.paint(canvas, Offset(xPosition, yPosition + (scheduleViewSettings.weekHeaderSettings.height / 2 - _textPainter.height / 2)));
  }
  ...
  void _addMonthLabel(Canvas canvas, Size size) {
    ...
    final TextSpan span = TextSpan(
      ...
      style: scheduleViewSettings.monthHeaderSettings.monthTextStyle ?? const TextStyle(color: Colors.white, fontSize: 20, fontFamily: 'Roboto'),
    );
    ...
    if (scheduleViewSettings.monthHeaderSettings.textAlign == TextAlign.right || scheduleViewSettings.monthHeaderSettings.textAlign == TextAlign.end) {
      xPosition = size.width - _textPainter.width;
    ...
    if (isRTL) {
      xPosition = size.width - _textPainter.width - xPosition;
      if (scheduleViewSettings.monthHeaderSettings.textAlign == TextAlign.left || scheduleViewSettings.monthHeaderSettings.textAlign == TextAlign.end) {
        xPosition = 0;
      } else if (scheduleViewSettings.monthHeaderSettings.textAlign == TextAlign.center) {
        xPosition = size.width / 2 - _textPainter.width / 2;
      }
...
class _AgendaDateTimePainter extends CustomPainter {
  _AgendaDateTimePainter(this.selectedDate, this.monthViewSettings, this.scheduleViewSettings, this.todayHighlightColor, this.todayTextStyle, this.locale, this.calendarTheme, this.agendaDateNotifier, this.viewWidth, this.isRTL, this.textScaleFactor, this.isMobilePlatform) : super(repaint: agendaDateNotifier);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (monthViewSettings != null) {
      ...
      dayTextStyle = scheduleViewSettings!.dayHeaderSettings.dayTextStyle ?? (useMobilePlatformUI ? calendarTheme.agendaDayTextStyle : TextStyle(color: calendarTheme.agendaDayTextStyle!.color, fontSize: 9, fontFamily: 'Roboto', fontWeight: FontWeight.w500));
      dateTextStyle = scheduleViewSettings!.dayHeaderSettings.dateTextStyle ?? (useMobilePlatformUI ? calendarTheme.agendaDateTextStyle : TextStyle(color: calendarTheme.agendaDateTextStyle!.color, fontSize: 18, fontFamily: 'Roboto', fontWeight: FontWeight.normal));
    }
    ...
    if (isToday) {
      ...
      dayTextStyle = todayTextStyle != null ? todayTextStyle!.copyWith(fontSize: dayTextStyle!.fontSize, color: todayTextColor) : dayTextStyle!.copyWith(color: todayTextColor);
      dateTextStyle = todayTextStyle != null ? todayTextStyle!.copyWith(fontSize: dateTextStyle!.fontSize, color: todayTextStyleColor) : dateTextStyle!.copyWith(color: todayTextStyleColor);
    }
  ...
  void _addDayLabelForMobile(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday, bool isMobile) {
    //// Draw Weekday
    final String dayTextFormat = scheduleViewSettings != null ? scheduleViewSettings!.dayHeaderSettings.dayFormat : 'EEE';
    TextSpan span = TextSpan(text: DateFormat(dayTextFormat, locale).format(selectedDate!).toUpperCase(), style: dayTextStyle);
    _updateTextPainter(span);
    ...
    if (agendaDateNotifier.value != null && isSameDate(agendaDateNotifier.value!.hoveringDate, selectedDate)) {
      if (xPosition < agendaDateNotifier.value!.hoveringOffset.dx && xPosition + _textPainter.width > agendaDateNotifier.value!.hoveringOffset.dx && yPosition < agendaDateNotifier.value!.hoveringOffset.dy && yPosition + _textPainter.height > agendaDateNotifier.value!.hoveringOffset.dy) {
        _linePainter.color = isToday ? Colors.black.withOpacity(0.1) : (calendarTheme.brightness == Brightness.dark ? Colors.white : Colors.black87).withOpacity(0.04);
        _drawTodayCircle(canvas, xPosition, yPosition, padding);
      }
    }
  ...
  void _addDayLabelForWeb(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday) {
    ...
    final String dayText = DateFormat(isRTL ? '${scheduleViewSettings!.dayHeaderSettings.dayFormat}, MMM' : 'MMM, ${scheduleViewSettings!.dayHeaderSettings.dayFormat}', locale).format(selectedDate!).toUpperCase();
    ...
    if (agendaDateNotifier.value != null && isSameDate(agendaDateNotifier.value!.hoveringDate, selectedDate)) {
      if (dateTextStartPosition < (isRTL ? size.width - agendaDateNotifier.value!.hoveringOffset.dx : agendaDateNotifier.value!.hoveringOffset.dx) &&
          (dateTextStartPosition + _textPainter.width) > (isRTL ? size.width - agendaDateNotifier.value!.hoveringOffset.dx : agendaDateNotifier.value!.hoveringOffset.dx) &&
  ...
  void _drawTodayCircle(Canvas canvas, double xPosition, double yPosition, double padding) {
    canvas.drawCircle(Offset(xPosition + (_textPainter.width / 2), yPosition + (_textPainter.height / 2)), _textPainter.width > _textPainter.height ? (_textPainter.width / 2) + padding : (_textPainter.height / 2) + padding, _linePainter);
  }
